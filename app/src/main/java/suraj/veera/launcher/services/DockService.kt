package suraj.veera.launcher.services
...
import suraj.veera.launcher.activities.LAUNCHER_ACTION
import suraj.veera.launcher.activities.LAUNCHER_RESUMED
import suraj.veera.launcher.activities.MainActivity
import suraj.veera.launcher.adapters.AppActionsAdapter
import suraj.veera.launcher.adapters.AppAdapter
import suraj.veera.launcher.adapters.AppAdapter.OnAppClickListener
import suraj.veera.launcher.adapters.AppShortcutAdapter
import suraj.veera.launcher.adapters.AppTaskAdapter
import suraj.veera.launcher.adapters.DisplaysAdapter
import suraj.veera.launcher.adapters.DockAppAdapter
import suraj.veera.launcher.adapters.DockAppAdapter.OnDockAppClickListener
import suraj.veera.launcher.db.DBHelper
...
import suraj.veera.launcher.models.DockApp
import suraj.veera.launcher.preferences.NAV_LONG_ACTIONS
import suraj.veera.launcher.receivers.BatteryStatsReceiver
import suraj.veera.launcher.receivers.SoundEventsReceiver
import suraj.veera.launcher.utils.AppUtils
import suraj.veera.launcher.utils.ColorUtils
import suraj.veera.launcher.utils.DeepShortcutManager
import suraj.veera.launcher.utils.DeviceUtils
import suraj.veera.launcher.utils.IconPackUtils
import suraj.veera.launcher.utils.OnSwipeListener
import suraj.veera.launcher.utils.Utils
import suraj.veera.launcher.widgets.HoverInterceptorLayout
...
class DockService : AccessibilityService(), OnSharedPreferenceChangeListener, OnTouchListener,
    ...
    private var startMenuView: android.view.View? = null
    private var isStartMenuShowing = false

    private fun toggleStartMenu() {
        if (isStartMenuShowing) {
            hideStartMenu()
        } else {
            showStartMenu()
        }
    ...
    private fun showStartMenu() {
        if (startMenuView == null) {
            startMenuView = LayoutInflater.from(this).inflate(R.layout.start_menu, null)
            ...
            val rvPinned = startMenuView?.findViewById<androidx.recyclerview.widget.RecyclerView>(R.id.rv_pinned_apps)
            rvPinned?.layoutManager = androidx.recyclerview.widget.GridLayoutManager(this, 4)
            rvPinned?.adapter = suraj.veera.launcher.adapters.DockAppAdapter(this, java.util.ArrayList(pinnedApps.map { suraj.veera.launcher.models.DockApp(it.name, it.packageName, it.icon) }), this, iconPackUtils)

            val btnPower = startMenuView?.findViewById<android.widget.ImageView>(R.id.btn_power)
            btnPower?.setOnClickListener {
                val intent = android.content.Intent(this, suraj.veera.launcher.activities.FileExplorerActivity::class.java)
                intent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(intent)
                hideStartMenu()
            }

            startMenuView?.setOnTouchListener { _, event ->
            ...
            windowManager.addView(startMenuView, params)
        } else {
            startMenuView?.visibility = android.view.View.VISIBLE
        }
        isStartMenuShowing = true
    }

    private fun hideStartMenu() {
        startMenuView?.visibility = android.view.View.GONE
        isStartMenuShowing = false
    }
    ...
    private fun createDock() {
        ...
        appsBtn.setOnClickListener { toggleStartMenu() }
